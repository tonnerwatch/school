...
<head>
<meta charset="utf-8" />
<title>ภาพรวมผู้พิการของโรงเรียน <?php echo $data['school_name_th'] . " ปี " . $year; ?></title>
...
<span class="caption-subject font-dark bold uppercase">กราฟแสดงภาพรวมผู้พิการของ<?php echo "โรงเรียน" . $data['school_name_th'] . " ปี " . $year; ?></span>
</div>
</div>
    ...
    <table class="table-bordered table-hover" style="margin: 50px auto 50px auto;">
		...
		$sql = "SELECT COUNT(student_id) AS STUDENT FROM student WHERE school_id = '" . $_SESSION['school_id'] . "' and (d1 or d2 or d3 or d4 or d5 or d6 or d7 or d8 or d9) = '1'";
		$result = mysqli_query($link, $sql);
		$data = mysqli_fetch_assoc($result);
...
<?php
$sql = "SELECT SUM(d1) AS D1, SUM(D2) AS D2, SUM(d3) AS D3, SUM(d4) AS D4, SUM(d5) AS D5, SUM(d6) AS D6, SUM(d7) AS D7, SUM(d8) AS D8, SUM(d9) AS D9, COUNT(student_id) AS STUDENT FROM student WHERE school_id = '" . $_SESSION['school_id'] . "'and year = '" . $year . "'";
$result = mysqli_query($link, $sql);
