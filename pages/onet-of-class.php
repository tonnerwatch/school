...
<head>
<meta charset="utf-8" />
<title>ผลการทดสอบทางการศึกษาระดับชาติขึ้นพื้นฐาน ชั้น <?php echo $id; ?></title>
...
<span class="caption-subject font-dark bold uppercase">กราฟแสดงผลการทดสอบทางการศึกษาระดับชาติขั้นพื้นฐาน <?php echo $id . " โรงเรียน" . $data['school_name_th']; ?></span>
</div>
</div>
<table width="40%" height="70" class="table-bordered table-hover" style="margin: 0 auto;">
<thead style="background-color: deepskyblue;">
<tr>
    <th width="5%" class="center">รหัส</th>
<th width="55%" class="center">วิชา</th>
<th width="20%" class="center">คะแนน(O-NET)</th>
<th width="20%" class="center">เปอร์เซ็น(%)</th>
</tr>
</thead>
<tbody>
<tr>
...
<?php
	...
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'ภาษาไทย'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$thai_language = @($score['SCORE'] / $score['CLASS']);
	
	
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'คณิตศาสตร์'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$math = @($score['SCORE'] / $score['CLASS']);
	
	
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'วิทยาศาสตร์'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$science = @($score['SCORE'] / $score['CLASS']);
	
	
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'ศิลปะ'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$art = @($score['SCORE'] / $score['CLASS']);
	
	
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'การงานอาชีพและเทคโนโลยี'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$career = @($score['SCORE'] / $score['CLASS']);
	
	
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'สังคมศึกษา ศาสนา และวัฒนธรรม'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$social = @($score['SCORE'] / $score['CLASS']);
	
	
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'ภาษาอังกฤษ'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$english = @($score['SCORE'] / $score['CLASS']);
	
	
	$sql = "SELECT SUM(onet.max_score) AS MAX, COUNT(onet.class) AS CLASS, SUM(onet.score) AS SCORE FROM onet inner join student on onet.id_card = student.id_card WHERE student.school_id = '" . $_SESSION['school_id'] . "' and student.class = '" . $_SESSION['class'] . "' and onet.year = '" . $year . "' and subject = 'สุขศึกษาและพลศึกษา'";
	$result = mysqli_query($link, $sql);
	$score = mysqli_fetch_assoc($result);
	$health = @($score['SCORE'] / $score['CLASS']);
	
	
	$sum_score = $thai_language + $math + $science + $art + $career + $social + $english + $health;
	
	$thai_persen = @(($thai_language * 100) / $sum_score);
	$math_persen = @(($math * 100) / $sum_score);
	$science_persen = @(($science * 100) / $sum_score);
	$art_persen = @(($art * 100) / $sum_score);
	$career_persen = @(($career * 100) / $sum_score);
	$social_persen = @(($social * 100) / $sum_score);
	$english_persen = @(($english * 100) / $sum_score);
	$health_persen = @(($health * 100) / $sum_score);
	
	$sum_persen = $thai_persen + $math_persen + $science_persen + $art_persen + $career_persen + $social_persen + $english_persen + $health_persen;
?>
<td class="center"> <?php echo number_format($thai_language, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($thai_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
<tr>
    <th class="center">A2</th>
<td width="200"> คณิตศาสตร์ </td>
<td class="center"> <?php echo number_format($math, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($math_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
<tr>
    <th class="center">A3</th>
<td width="200"> วิทยาศาสตร์ </td>
<td class="center"> <?php echo number_format($science, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($science_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
<tr>
    <th class="center">A4</th>
<td width="200"> ศิลปะ </td>
<td class="center"> <?php echo number_format($art, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($art_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
<tr>
    <th class="center">A5</th>
<td width="200"> การงานอาชีพและเทคโนโลยี </td>
<td class="center"> <?php echo number_format($career, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($career_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
<tr>
    <th class="center">A6</th>
<td width="200"> สังคมศึกษา ศาสนา และวัฒนธรรม </td>
<td class="center"> <?php echo number_format($social, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($social_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
<tr>
    <th class="center">A7</th>
<td width="200"> ภาษาอังกฤษ </td>
<td class="center"> <?php echo number_format($english, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($english_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
<tr>
    <th class="center">A8</th>
<td width="200"> สุขศึกษาและพลศึกษา </td>
<?php
	
	
?>
<td class="center"> <?php echo number_format($health, 2, '.', ' '); ?> </td>
<td class="center"> <?php echo number_format($health_persen, 2, '.', ' ') . "%"; ?> </td>
</tr>
</tbody>
<tfoot>
<tr>
...
<td class="center"><?php echo number_format($sum_score, 2, '.', ' ');?></td>
<td class="center"><?php echo number_format($sum_persen, 2, '.', ' ') . "%";?></td>
</tr>
</tfoot>
</table>
<!-- BEGIN CHART PORTLET-->
...
<div id="chart_5" class="chart" style="height: 400px;width: 100%;max-width: 600px;margin: 0 auto;"> </div>
